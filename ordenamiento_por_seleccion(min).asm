...
main:
    ...
    jal sort_reverse # Llama a la función de ordenación reversa
...
sort_loop:
    ...
    # Intercambia el mínimo con el primer elemento de la parte no ordenada
    lw $t1, ($s0)        # $t1 = primer elemento de la parte no ordenada
    lw $t2, ($v0)        # $t2 = elemento mínimo
    sw $t2, ($s0)        # Coloca el mínimo al principio
    sw $t1, ($v0)        # Coloca el primer elemento donde estaba el mínimo
...
min:
    ...
    lw $t0, ($v0)        # $t0 = valor mínimo actual
...
min_loop:
    ...
    bge $t2, $t0, min_loop # Si es mayor o igual, continúa el bucle
    
    move $v0, $a0        # Actualiza la dirección del mínimo
    move $t0, $t2        # Actualiza el valor mínimo
    j min_loop
